*SAS program to estimate heritability from random lines evaluated in a randomized complete block
design with 3 replications at each of 5 environments, with some missing data;
data one;
...
proc mixed asycov; class env rep entry; model yield = ; 
...
*order of variance components in v and c matrices is V(E), V(R), V(G), V(GE), V(error);
LG = {0, 0, 1, 0, 0};
LP = {0, 0, 1, 1, 1};
